...
int test_account()
{
    ...
    m.aggiungi_utente(new Premium("dio","password","nome","cogmone","email"));
...
//        cout<<endl<<**it;
    ...
    Utente* dio=m.get_utente("dio");
    Utente* gesu=m.get_utente("gesu");
    Utente* maria=m.get_utente("maria");
    Account A(g,&m);
    ...
    A.aggiungi_amico(p2);
    A.aggiungi_amico(b);
    A.aggiungi_amico(dio);
    A.aggiungi_amico(maria);
    A.aggiungi_amico(gesu);
    p->aggiungi_amico(g);
    p->aggiungi_amico(p2);
    p->aggiungi_amico(b);
