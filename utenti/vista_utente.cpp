...
void VistaUtente::aggiungiAreaDomandaAmici()
{
   ...
   container<Domanda*> contenitoreDomandeAmici=a->get_domande_amici();
   ...
   QTextEdit* testoDomanda=new QTextEdit(QString::fromStdString(a->get_domanda(contenitoreDomandeAmici,i)->get_testo())); //al momento della creazione verrá inserito il testo della domanda
   ...
   bottoneVediCommento* commenti=new bottoneVediCommento(a->get_domanda(contenitoreDomandeAmici,i),"vedi commenti");
...
VistaUtente::VistaUtente(Account * ac, QWidget *parent):QWidget(parent),a(ac)
{
...
{
   vistaProfilo* profilo=new vistaProfilo(a);
